<?php
    ...
    if (isset($_GET['edit']))
    {
      $idmanage = $_GET['edit'];
      $update = true;
      $result = $mysqli->query("SELECT * FROM manage_duration_su where manage_duration_su_id='$idmanage'") or die($mysqli->error());
      if (count($result)==1)
      {
...
<form action="" method="post" enctype="multipart/form-data" id="validationform" data-parsley-validate="" novalidate="">
            <input type="hidden" name="manage_duration_su_id" value="<?php echo $idmanage; ?>">
            <div class="box-body">
  ...
  if (isset($_POST['updateM'])){
    ...
    $idmanage = $_POST['manage_duration_su_id'];
    $Sdate = $_POST['Sdate']; 
    $Edate = $_POST['Edate'];
    $txt = $_POST['txt'];

      $mysqli->query("UPDATE manage_duration_su SET Sdate = '$Sdate', Edate = '$Edate', txt = '$txt' WHERE manage_duration_su_id=$idmanage") or die($mysqli->error);
  ?>
