...
class PDF extends FPDF{
  ...
  function Header()  {
        ...
        function viewTable($db)
        {
          ...
          $stmt = $db->query('select * from su');
          while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
            $data->vote = base64_decode($data->vote);
            //$this->Cell(20,10,$data->image,1,0,'C');
            $this->Cell(30,10,$data->Sname,1,0,'C');
            $this->Cell(50,10,$data->Semail,1,0,'C');
            $this->Cell(35,10,$data->Sphone,1,0,'C');
            $this->Cell(60,10,$data->Sclub,1,0,'C');
